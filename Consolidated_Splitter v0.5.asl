...
startup
{
	...
	settings.Add("campaign", true, "Campaign");
		settings.Add("allmissions", true, "All Missions", "campaign");
			...
			settings.Add("tramtimer", true, "Tram Timer?", "amsubsplit");
			...
			settings.Add("ManualSubsplit", false, "Additional Subsplits", "amsubsplit");
				...
				settings.Add("m7Elevator", false, "M7 Vent Drop", "ManualSubsplit");
				...
				settings.Add("m16Minigames", false, "M16 Minigames", "ManualSubsplit");
	...
	settings.Add("ils", false, "ILs");
		settings.Add("ilsSub", false, "IL Subsplit", "ils");
			...
			settings.Add("ILManualSubsplit", false, "Additional Subsplits", "ilsSub");
				...
				settings.Add("ILm7Drop", false, "M7 Vent Drop", "ILManualSubsplit");
				...
				settings.Add("ILm16Minigames", false, "M16 Minigames", "ILManualSubsplit");
...
start
{
	...
	if(settings["allmissions"] == true && vars.fadetrigger){ // Detects if PLAYER is starting All Missions
	print("Debug Print: Starting All Missions");
		vars.SavedMap = current.Map;
		vars.repeatstopper = 0;
		return true;}
	else if(settings["any"] == true && vars.fadetrigger){ // Detects if PLAYER is starting Any% 
	print("Debug Print: Starting ANY%");
		vars.SavedMap = current.Map;
		vars.repeatstopper = 0;
		return true;}
	...
	else if(settings["ils"] == true && vars.fadetrigger && current.missionNum == 19 || settings["ils"] == true && vars.fadetrigger){
		vars.final = true;
		return true;}
}
...
update
{	// Subsplit variables
	vars.m2Elevator = current.pX > 22.85 && current.pX < 26.11 && current.pZ > -108 && current.pZ < -104.5 && current.pY < 5;
	...
	vars.M6ColdStorage = current.pY > 1 && current.pY < 2 && current.pZ > -23 && current.pZ < -20 && current.pX > -10 && current.pX < -8;
	//vars.m6Medbay = current.pX > 52 && current.pX < 53 && current.pZ > -46 && current.pZ < -44;
	vars.m7Elevator = current.pZ > 58.5 && current.pZ < 58.7 && current.pX > -22.55 && current.pX < -22.4 && current.pY > -15 && current.pY < -14.9;
	vars.m8Elevator = current.pX > 31.2 && current.pX < 33.2 && current.pZ > 68.5 && current.pZ < 71 && current.pY > 9 && current.pY < 10;
	vars.m12vent = current.pX > -48.5 && current.pX < -47 && current.pZ > 38.5 && current.pZ < 39 && current.pY > 19.7; 
	vars.m12ElevatorUp = current.pZ > 46.3 && current.pZ < 50.1 && current.pX > -30 && current.pX < -25 && current.pY > 19 && current.pY < 19.5;
	vars.m12ElevatorDwn = current.pZ > 46.3 && current.pZ < 50.1 && current.pX > -30 && current.pX < -25 && current.pY > 4 && current.pY < 4.2;
	vars.m13ventDrop = current.pZ > -2.6 && current.pZ < -1.1 && current.pX > -17.5 && current.pX < -15 && current.pY > 0.8 && current.pY < 1.2;
	vars.m14ventDrop = current.pZ > -44 && current.pZ < -42 && current.pX > -24 && current.pX < -21 && current.pY > 16.3 && current.pY < 16.8;
	vars.m16Ladder = current.pY > 28.5 && current.pY < 28.8 & current.pZ > 56.9 && current.pZ < 57.1 && current.pX > -38.8 && current.pX < -38.6;
	vars.m17TransitElevator = current.pX > 52.4 && current.pX < 52.55 && current.pZ > 5.84 && current.pZ < 5.86;
	vars.m18VentGrab = current.pX >= -105 && current.pX <= -103 && current.pZ >= 93 && current.pZ <= 96;
	vars.m18SpaceWalk = current.pX >= 21.4 && current.pX <= 22 && current.pY >= -20.8 && current.pY <= -20.6 && current.pZ >= -1 && current.pZ <= 0;
	vars.m2Tram = current.pX > 23.7 && current.pX < 24.1 && current.pZ > 52.16 && current.pZ < 52.18;
	...
	if (vars.mission == null && current.fadeState == 2 && old.fadeNum < 0.5 && current.fadeNum > 0.5){vars.mission = current.missionNum;} // Updates vars.mission with the current mission number when game starts
}
...
split
{
	...
	// Splits for Any% CC Only
	if (settings["any"] == true){
		if (current.Map != vars.SavedMap){
			vars.SavedMap = current.Map;
			return true;}
		else if (current.missionNum == 18 && current.pX >= -105 && current.pX <= -103 && current.pZ >= 93 && current.pZ <= 96 && vars.repeatstopper == 0){
			vars.repeatstopper = 1;
			return true;}
		else if (current.missionNum == 18 && current.pX >= 21.4 && current.pX <= 22 && current.pY >= -20.8 && current.pY <= -20.6 && current.pZ >= -1 && current.pZ <= 0 && vars.repeatstopper == 1){
	...
	// Splits for Subsplits (First Statement is 'Split on Map Change'.)
	if(settings["allmissions"] == true && settings["yesSubsplit"] == true || settings["ilsSub"] == true && settings["ILyesSubsplit"] == true){
		if (vars.SavedMap != current.Map || current.missionNum > vars.mission){
			vars.repeatstopper = 0;
			vars.mission = current.missionNum;
			vars.SavedMap = current.Map;
			return true;}
	}
	// Splits for Tram Timer
	...
	if(settings["m16ElevatorUp"] == true || settings["ILm16ElevatorUp"] == true){
		if(current.missionNum == 16 && vars.m4ElevatorUp && vars.repeatstopper == 0){ // M16 Elevator Up
			vars.repeatstopper = 1;
			return true;}}
	if(settings["m16Minigames"] == true || settings["ILm16Minigames"] == true){
		{if(current.missionNum == 16 && vars.m16Ladder && vars.repeatstopper == 1){ // M16 Elevator Up
			vars.repeatstopper = 2;
			return true;}}
		{if(current.missionNum == 16 && vars.m16Ladder && vars.repeatstopper == 2){ // M16 Elevator Up
			vars.repeatstopper = 3;
			return true;}}}
